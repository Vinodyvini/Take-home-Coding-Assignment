export default defineContentScript({
  // Matches LinkedIn URL pattern
  matches: ['*://*.linkedin.com/*'],
  async main() {
    console.log('Content script running on LinkedIn.');

    // Function to insert text into LinkedIn's message input
    ...
    // Declaration of Variable for aiicon and modal.
    let aiIcon: HTMLDivElement | null = null;
    let modal: HTMLDivElement | null = null;
    let secondModal: HTMLDivElement | null = null;
    // Function to create the first modal window
    const createFirstModal = () => {
      // Modal overlay with background
      ...
      `;
      // Append modal content to overlay and add it to the document body
      ...
      generateBtn?.addEventListener('click', () => {
        if (firstInput.value) {
          ...
          createSecondModal(firstInput.value);// Open the second modal with input value
        }
      });

      modal = modalOverlay; // Store reference to the modal
    };
    // Function to create the second modal
    const createSecondModal = (prompt: string) => {
      // Modal overlay for the second modal
      ...
      // Modal content with dynamic prompt
      ...
      `;
      // Append second modal content to overlay and add to document
      secondModalOverlay.appendChild(secondModalContent);
      document.body.appendChild(secondModalOverlay);
      // Close second modal on clicking outside
      ...
      // Insert button click event to add final text into LinkedIn message
      insertBtn?.addEventListener('click', () => {
        ...
        insertTextIntoLinkedIn(finalText);// Insert final message
        closeSecondModal();// Close the second modal
      });
      // Regenerate button click event (currently just logs a message)
      regenerateBtn?.addEventListener('click', () => {
        console.log('Regenerate button clicked');
      });

      secondModal = secondModalOverlay; // Store reference to the second modal
    };
    ...
    // Function to insert AI icon next to the message input
    const insertAiIcon = () => {
      const messageInput = document.querySelector('.msg-form__contenteditable');
      // If the message input field is not found, log an error and return
      if (!messageInput) {
        console.log('Message input not found');
        return;
      }
      // If the AI icon does not already exist, create it
      if (!aiIcon) {
